...
public:
	...
	void runSimulation(board &B, int &win) {
		...
		if (curState == -1) {
			...
			int seq = rand() % sz;
			int n = 0;
			for (vector<position>::iterator iter = currentNodes.begin(); iter != currentNodes.end(); iter++) {
				if (n == seq) {
	...
	void traditionalSelect(board &B, int aiPlayer) {
		vector<position> validp;
		board C(B);
		int state = C.getStatus(validp);
		if (state != -1) { // game is over
			getResult(B, state); //choose winner
			return;
		}
		...
		if (aiPlayer != B.curPlayer) {
			...
			while (curTime < beginTime + simulationTime / 1000) {
				for (vector<position>::iterator iter = validp.begin(); iter != validp.end(); iter++) {
		...
		if (aiPlayer != B.curPlayer) {
			...
			while (curTime < beginTime + simulationTime / 1000) {
				...
				for (vector<position>::iterator iter = validp.begin(); iter != validp.end(); iter++) {
					...
					int i = 0;
					for (vector<position>::iterator iterin = validp.begin(); iterin != validp.end(); iterin++) {
						i++;
						statisticsConfidenceIntervals[i] = (double)(moves[i]) / cnt + sqrt(log(N[i]) / cnt);
