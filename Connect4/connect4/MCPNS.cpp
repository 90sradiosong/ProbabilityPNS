	...
	else
	{
		...
		while (1)
		{
			...
			if (nodeNum >= Limit || time_elapsed >= 420) {
				...
				result.append(";\n");
				writeIntoFile("MCPNSTresult.txt", result);
				return 0;
			}
			search(&r, *playerAI);
			if (itnum % 10000 == 0) {
				...
				result.append(std::to_string(r.dn));
				result.append(";\n");
				writeIntoFile("MCPNSTPNDN.txt", result);
			}
			if (r.pn == 0)
			{
				solved = true;
...
void MCPNS::getPNDN(Node *p, playerInfo& activePlayer)
{
	...
	p->dn = (double)wnum / MaxSimulationNum;
	p->pn = 1.0 - (double)wnum / MaxSimulationNum;
}

void MCPNS::search(Node* p, playerInfo& activePlayer)
{
	if (p->child.size() == 0)
	{
		//if (p == &r)
