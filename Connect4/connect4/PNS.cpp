	...
	else
	{
		...
		while (1)
		{
			...
			if (nodeNum >= Limit || time_elapsed>=420) {
				...
				result.append(";\n");
				writeIntoFile("PNSTresult.txt", result);
				return 0;
			}
			if (itnum % 10000 == 0) {
				...
				result.append(std::to_string(r.dn));
				result.append(";\n");
				writeIntoFile("PNSTPNDN.txt", result);
			}
			search(&r, *playerAI);
			if (r.pn == 0)
			{
...
void PNS::getPNDN(Node *p, playerInfo& activePlayer)
{
...
void PNS::search(Node* p, playerInfo& activePlayer)
{
	if (p->child.size() == 0)
	{
		//if (p == &r)
